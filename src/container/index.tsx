...
import { IAppInfo, IEnvInfo, IScreen } from './interface'
		...
		try {
			...
			if (data.type && typeof data.type === 'string') {
				switch (data.type.toUpperCase()) {
					case 'LOG':
						window.main?.sendLog(data.level || 'INFO', data.payload)
						// window.main?.send
            //   'child.action',
            //   'log',
            //   data.level || 'INFO',
...
window.main?.receive('app_infos', (appInfos: IAppInfo) => {
	store.dispatch(setAppInfos(appInfos))

})

window.main?.receive('request_wpt.done', (action: string, data: any) => {
	store.dispatch(setAskAction(false))
	switch (action) {
		case 'plugins':
...
window.main?.receive('conf', (conf: any) => {
	...
	if (conf.zoom && window.webFrame) {
		if (conf.zoom.level) {
			window.webFrame.setZoomLevel(conf.zoom.level)
		}
		if (conf.zoom.factor) {
			window.webFrame.setZoomFactor(conf.zoom.factor)
		}
	}
})
...
window.main?.receive('ask_password', () => {
	// eslint-disable-next-line @typescript-eslint/no-unused-vars
...
window.main?.receive('notification', (notif: any) => {
...
window.main?.receive('menu.action', (action: TNextAction) => {
...
const onCallback = (action: TNextAction, ...data: any) => {
	...
	switch (action) {
		case TNextAction.EMERGENCY:
			window.main?.send
			window.main?.send('main.action', 'emergency')
			break
		case TNextAction.CLOSE:
			window.main?.send
			window.main?.send('main.action', 'close')
			break
		case TNextAction.RELOAD:
			store.dispatch(iFrameReadyAction(false))
			window.main?.send
			window.main?.send('main.action', 'reload')
			break
		case TNextAction.REQUEST_WPT:
			store.dispatch(setAskAction(true))
			// window.main?.send
			window.main?.send('main_action', 'plugins')
			if (data && data.length > 0) {
				const keyMessage: string = data.shift()
				window.main?.send
				window.main?.send('request_wpt', keyMessage, ...data)
			}
			break
		case TNextAction.REPORT:
			...
			if (api_key) {
				...
				window.main?.send('request_wpt', 'fastprinter.defaultprinterdata')
				axios
		...
		case TNextAction.OPEN_DEV_TOOLS:
			window.main?.send
			window.main?.send('main.action', 'open_dev_tools')
			break
		default:
			break
	}
